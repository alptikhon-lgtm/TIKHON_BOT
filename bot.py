...
def askDeepseek(question):
    ...
    data = {
        ...
        "messages": [
            {
                "role": "system",
                "content": "Ты полезный помощник. Отвечай максимально кратко и по делу. Ограничь ответ 7-8 предложениями. Используй не более 1200-1500 букв в ответе"
            },
            {
                "role": "user",
                "content": question
...
def start(message):
    welcome_text = "🤖 Это бот-дипсик Алпеева Тихона. Задай вопрос и получи  ответ.\n\nИспользуй команду:\n/ai [твой вопрос]\n\nПример: /ai Что такое искусственный интеллект?""узнай что я делаю командой /info"
    bot.send_message(message.chat.id, welcome_text)
    bot.send_sticker(message.chat.id, "CAACAgIAAxkBAAEQTJlpchulxRKfRUQeYR-nXc5fLbwJdAAC2A8AAkjyYEsV-8TaeHRrmDgE")


@bot.message_handler(commands=["info"])
def info(m):
    bot.send_message(m.chat.id, "Список команд бота:\n"
                                "/img(что ты хочешь на пример /img car)(пиши на англ.)-генерация картинок"
                                "/ai ответ на любой вопрос")
...
def deepseekSearch(message):
    ...
    if not check_daily_limit(user_id):
        bot.send_message(
            message.chat.id,
            "❌ Вы превысили дневной лимит в 30 вопросов. Попробуйте завтра!"
        )
        return
